...
public class PlayerH2Service implements PlayerRepository {
    ...
    public ArrayList<Player> getPlayers() {
        List<Player> playerNum = db.query("select * from team", new PlayerRowMapper());

        ArrayList<Player> players = new ArrayList<>(playerNum);
        return players;
    }
    ...
    public Player addPlayer(Player player) {
        ...
        db.update("insert team into(playerName,jerseyNumber,role)values(?,?,?)", player.getPlayerName(),
                player.getJerseyNumber(), player.getRole());
        Player savedPlayer = db.queryForObject(
                "select * from team where playerName = ?, jerseyNumber = ? and role = ?", new PlayerRowMapper(),
